...
import { useCreateGoodsIssueManual, useGoodsIssueList } from '@/features/goods-issues/api'
...
import type { GoodsIssue } from '@/features/goods-issues/api'
...
const columns: Column<GoodsIssue>[] = [
  {
    ...
    header: 'Nomor',
    cell: (r) => (
      <Link to={`/goods-issues/${r.id}`} className="font-mono text-xs text-primary hover:underline">
        {r.number}
      </Link>
    ),
  },
...
export function GoodsIssueListPage() {
  ...
  const { data, isLoading } = useGoodsIssueList({ search: search || undefined, status: status || undefined, page })
  ...
  const create = useCreateGoodsIssueManual()
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        subtitle="Pickup barang dari request (dulu disebut NPBG di halaman ini)"
        icon={<ScrollText className="size-5" />}
        actions={
          hasPermission('goods_issue.create') ? (
            <Button size="sm" variant="outline" onClick={() => setShowForm((v) => !v)}>
              {showForm ? 'Tutup' : 'Buat Manual'}
            </Button>
          ) : undefined
        }
      />
