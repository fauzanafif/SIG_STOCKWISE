...
import {
  usePurchaseProposal,
  usePurchaseProposalAction,
  useDeletePurchaseProposal,
  useUpdatePurchaseProposal,
...
const lineColumns: Column<PurchaseProposalLine>[] = [
...
function EditForm({ ppb, onDone }: { ppb: PurchaseProposal; onDone: () => void }) {
  const update = useUpdatePurchaseProposal(ppb.id)
...
export function PurchaseProposalDetailPage() {
  const { id } = useParams()
  const navigate = useNavigate()
  const ppbId = Number(id)
  const { hasPermission } = useAuth()
  const { data: ppb, isLoading } = usePurchaseProposal(ppbId)
  const action = usePurchaseProposalAction(ppbId)
  const del = useDeletePurchaseProposal()
  const [err, setErr] = useState<string | null>(null)
  ...
  function remove() {
    if (!window.confirm(`Hapus usulan pembelian ${ppb!.number}?`)) return
    del.mutate(ppb!.id, { onSuccess: () => navigate('/purchase-proposals'), onError: (e) => setErr(apiErrorMessage(e)) })
  }
  ...
  return (
    <div className="max-w-4xl space-y-4">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {ppb.source_request_number ? `dari ${ppb.source_request_number}` : 'Dibuat manual'}
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {ppb.status === 'DRAFT' && hasPermission('purchase_proposal.update') && (
          <>
        ...
        {ppb.status === 'DRAFT' && hasPermission('purchase_proposal.submit') && (
          <Button size="sm" onClick={() => run('submit')}>
            Submit
          </Button>
        )}
        {['SUBMITTED', 'REVIEW'].includes(ppb.status) && hasPermission('purchase_proposal.review') && (
          <Button size="sm" variant="outline" onClick={() => run('review')}>
            Tandai Direview
          </Button>
        )}
        {['SUBMITTED', 'REVIEW'].includes(ppb.status) && hasPermission('purchase_proposal.approve') && (
          <Button size="sm" onClick={() => run('approve')}>
            Approve
          </Button>
        )}
        {['SUBMITTED', 'REVIEW'].includes(ppb.status) && hasPermission('purchase_proposal.reject') && (
