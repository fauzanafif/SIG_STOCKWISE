...
import { useCreatePpb, usePpbList, type Ppb } from '@/features/purchasing/api'
...
export function PpbListPage() {
  ...
  const { data, isLoading } = usePpbList({ search: search || undefined, status: status || undefined, page })

  const [showForm, setShowForm] = useState(false)
  const [lines, setLines] = useState<DraftLine[]>([])
  const [notes, setNotes] = useState('')
  const [err, setErr] = useState<string | null>(null)
  const create = useCreatePpb()
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="PPB — Permintaan Pembelian Barang"
        subtitle="Usulan pembelian dari kekurangan stok"
        icon={<FileSpreadsheet className="size-5" />}
        actions={
          hasPermission('ppb.create') ? (
            <Button size="sm" variant="outline" onClick={() => setShowForm((v) => !v)}>
              {showForm ? 'Tutup' : 'Buat PPB Manual'}
            </Button>
          ) : undefined
        }
      />
